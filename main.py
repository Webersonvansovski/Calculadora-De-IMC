...
def IMC():
    ...
    # converter Height(altura) em metros
    m = h/100
    imc = round(float(w/m**2), 1)
    label1.config(text=imc)

    if imc <= 18.5:
        label2.config(text="Abaixo !")
        label3.config(text="Você esta abaixo do peso recomendado!")
    elif imc > 18.5 and imc <= 25:
        label2.config(text="Normal!")
        label3.config(text="Você esta saudável e no peso recomendado!")
    elif imc > 25 and imc <= 30:
        label2.config(text="Acima!")
        label3.config(text="Você esta acima do peso recomendado! \n um médico pode recomendar \n a perder um pouco de peso \n por razoes de saúde!")
    else:
        label2.config(text="Obeso!")
        label3.config(text="Sua saúde está em risco! \n procure um médico imediatamente!")
...
# comando para mudar a cor de fundo da escola
style = ttk.Style()
